...
AI가 매일 새로운 픽셀 친구를 만들어 주는 감성 마이크로 저널 앱 **Pixel Grug**의 제품 소개 랜딩페이지입니다. 순수 HTML/CSS/JS로 제작되었으며, **Anthropic Light / Dark Theme**와 다국어 전환을 지원합니다.
...
├── js/
│   └── main.js          # 모바일 메뉴, 스크롤 리빌, 다국어/테마 토글, Daily Pixel 데모
...
CSS 커스텀 프로퍼티로 라이트/다크 테마 디자인 토큰을 정의했습니다.

| 항목 | 값 |
| --- | --- |
| 라이트 배경 | 웜 아이보리 `#F0EEE6` / 카드 `#FAF9F5` |
| 다크 배경 | 차분한 웜 다크 `#1B1A17` / 카드 `#2A2824` |
| 텍스트 | 라이트 잉크 `#1A1915` / 다크 잉크 `#F5F0E7` |
| 액센트 | 클레이·코랄 `#CC785C` / 다크 변형 `#E29374` |
| 폰트 | 헤딩 Fraunces(serif) · 본문 Inter(sans) · 강조 Press Start 2P(pixel) |
| 스타일 | 넉넉한 여백, 소프트 라운드, 미묘한 그림자, 토글 기반 테마 전환 |

## 다국어 지원

- **지원 언어** — 한국어 / 영어 / 중국어 / 일본어
- **언어 선택 UI** — 상단 네비게이션의 `🌐` 아이콘 기반 플래그 메뉴
- **저장 방식** — 선택한 언어를 `localStorage`에 저장
- **기본 언어 처리** — 저장값이 없으면 한국어 기본, 브라우저 언어도 함께 인식
...
- **언어 토글** — `🌐` 아이콘 메뉴에서 `ko / en / zh / ja` 전환
- **테마 토글** — `☀ / ☾` 아이콘 하나로 라이트/다크 전환
...
- **다국어 선택 메뉴에서 한국어 옵션 누락**
  - 증상: 언어 전환 UI에 영어/중국어/일본어만 보이고 한국어를 다시 선택할 수 없었음.
  - 해결: `ko`를 언어 메뉴와 스크립트 지원 목록에 복원하고, 기본 언어도 한국어 기준으로 정리.

- **라이트/다크 모드에서 상단 `Get Started` 버튼 hover 색상 일관성 문제**
  - 증상: 라이트 모드와 다크 모드에서 상단 CTA 버튼에 마우스를 올리면 배경색이 바뀌어 의도한 고정 스타일이 유지되지 않았음.
  - 해결: hover 시 배경색이 변하지 않도록 통일하고, 다크 모드에서는 텍스트 색상을 검정으로 맞춰 가독성과 시각 일관성을 유지.
